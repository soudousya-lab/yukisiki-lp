  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16 md:h-20">
        <a href="#" className="font-serif text-lg md:text-xl tracking-widest text-charcoal">
          YUKISIKI
        </a>
        ...
        <nav className="hidden lg:flex items-center gap-6">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="text-xs tracking-wider text-charcoal-light hover:text-gold transition-colors"
            >
              {l.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="#reserve"
            className="hidden md:inline-block bg-gold text-white text-xs tracking-wider px-6 py-2.5 rounded-full hover:bg-gold-dark transition-colors"
          >
            ご予約はこちら
          </a>
          <button
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="lg:hidden bg-warm-white/98 backdrop-blur-md border-t border-greige/30">
          <nav className="flex flex-col items-center py-6 gap-5">
            {links.map((l) => (
              <a
                ...
                className="text-sm tracking-wider text-charcoal-light hover:text-gold transition-colors"
              >
                {l.label}
              </a>
            ))}
