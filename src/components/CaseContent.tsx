...
export default function CaseContent() {
  ...
  const scrollToCase = useCallback((caseId: string) => {
    ...
    // CSSの scroll-behavior:smooth と JSのsmoothスクロールが競合し、
    // ScrollRevealの遅延表示と相まって先頭に戻る不具合があったため、
    // behavior:"auto"(instant)に統一。レイアウト確定後に再計測して補正。
    window.scrollTo({ top: compute(), behavior: "auto" });
    requestAnimationFrame(() => window.scrollTo({ top: compute(), behavior: "auto" }));
    // 画像読み込み等で高さが変わるケースの最終補正
    setTimeout(() => window.scrollTo({ top: compute(), behavior: "auto" }), 400);
  }, []);
  ...
  useEffect(() => {
    ...
    if (hash && hash.startsWith("case-")) {
      const timer = setTimeout(() => scrollToCase(hash), 300);
  ...
  return (
    <>
      ...
      <main>
        {/* ===== ヘッダー ===== */}
        <section className="bg-ivory pt-28 pb-8 md:pt-36 md:pb-10">
        ...
        {/* ===== スクロールインジケータ ===== */}
        <div className="bg-ivory flex flex-col items-center pb-8">
              ...
              <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-10">
                ...
                <ScrollReveal animation="fade-left" delay={100} className="w-full md:w-[62%]">
                  <div>
                    ...
                    <h2 className="font-serif text-lg md:text-xl tracking-wider text-charcoal mt-1 mb-4">
                      {c.title}
                    </h2>
                    ...
                    {c.sections.map((s, si) => (
                      <div key={si} className="mb-4">
                        <h3 className="text-xs md:text-sm font-medium text-charcoal mb-1.5">
                          【{s.heading}】
                        </h3>
